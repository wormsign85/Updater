...
$sql = "UPDATE wormsignh_haffner.products SET short_description=:short_description, updated_at=NOW() WHERE sku=:sku ";
$sql_select = "SELECT sku FROM products";
...
foreach ($products->product as $sorszam => $product) {
    ...
    $q->execute(array(
        ':sku' => $sku,
        ':short_description' => html_entity_decode(strtr($short_description, array('&lt,' => '<', '&gt,' => '>')))
    ));
}