...
$map = array();
$result = ata_mysql_query("SELECT * FROM wormsignh_haffner.gigatel WHERE new_main_category LIKE '%tokok%'");

while ($row = mysql_fetch_assoc($result)) {
    $jonekunk = jonekunk($row['compatible_devices_new']);
    if (!$jonekunk) {
        ata_mysql_query('UPDATE wormsignh_haffner.gigatel SET alt_cat="" WHERE cikkszam="' . addslashes($row['cikkszam']) . '"');
        continue;
    }
    ...
    $letrehoz = ata_mysql_query('UPDATE wormsignh_haffner.gigatel SET alt_cat="' . addslashes($altCatString) . '" WHERE cikkszam="' . addslashes($row['cikkszam']) . '"');
}
