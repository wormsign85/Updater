...
try {
    $conn = new PDO('mysql:host=localhost;dbname=wormsignh_kapacitas_sku', $user, $pass);
    ...
    $sku_select = "SELECT sku_original FROM sku_convert WHERE sku_new=:sku_new";
...
//$params = $order->Customer->Params->Param->Value; Ha a vásárló paraméterekből vesszük a vevőkódot
        ...
        foreach ($order->Items->Item as $itemKey => $item) {
            ...
            $sth = $conn->prepare($sku_select);
            $sth->bindParam(':sku_new', $Item_Sku);
            $sth->execute();
            $sku_new1 = $result = $sth->fetchColumn();

            $line_item->addChild('EAN', $sku_new1);
