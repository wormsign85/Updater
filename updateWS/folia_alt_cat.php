...
$map = array();
$result = ata_mysql_query("SELECT * FROM wormsignh_haffner.products WHERE categories LIKE '%képernyővédő fóliák%'");
...
while ($row = mysql_fetch_assoc($result)) {
    ...
    foreach ($termekek as $markaestipus) {
        if (jonekunk($markaestipus)) {
            ...
            if ($markaestipusarray) {
                $altCat[] = 'KERESO' . '|' . $markaestipusarray['marka'] . '|' . $markaestipusarray['tipus'] . '|' . 'LCD Védő Fólia és Üveg';
            }
        }
    }
    ...
    $letrehoz = ata_mysql_query('UPDATE wormsignh_haffner.products SET alt_cat="' . addslashes($altCatString) . '" WHERE sku="' . addslashes($row['sku']) . '"');
}
