...
$result = ata_mysql_query('SELECT sku, categories FROM wormsignh_haffner.products WHERE stock>=3');
...
//    }
    ...
    $xrefSql = 'SELECT x2.szla_id FROM wormsignh_haffner.product_xref x1'
        . ' INNER JOIN wormsignh_haffner.product_xref x2 ON (x1.xref_new=x2.xref_new AND x1.szla_id != x2.szla_id)'
        . ' WHERE x1.sku="' . addslashes($row['sku']) . '" AND (x2.stock > 0 OR x2.gt_stock="készleten") ' . $categorySqlPart . $categorySqlPartExclude . 
            ' AND x2.haszonkulcs>36 AND x2.category NOT LIKE ' . "'%töltő%'";
    echo $xrefSql . '<br>';
